...
| **Task artifact** — `*.caliper-task.json` v1 (scene + zones + gripper + success + horizon), stability-contract covered, Rust/Python parity-tested | [`caliper-sim-mujoco::task`](../reference/task-artifact.md) | ✗ ³ (`caliper-learn eval/autopsy --task`) | `caliper_learn.load_task` / `VecSimEnv.from_task` | Open task… (robot + scene + zones + live SUCCESS badge) |
| **Policy-in-the-loop drive** — trained checkpoint drives the live sim over a stdio JSON bridge | [`caliper_learn.bridge`](../capabilities/learning.md) | ✗ ³ (`caliper-learn drive`) | `drive_loop` / `load_drive_policy` | Simulate: connect policy… (live session; state-based policies) |
